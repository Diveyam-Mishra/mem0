...
export class MongoDB implements VectorStore {
  ...
  private async _doInitialize(): Promise<void> {
    try {
      ...
      if (collections.length === 0) {
        console.log(
          `Collection '${this.collectionName}' does not exist. Creating it now.`,
        );
      ...
      try {
        ...
        if (foundIndex) {
          console.log(
            `Search index '${this.indexName}' already exists in collection '${this.collectionName}'.`,
          );
        } else {
          ...
          console.log(
            `Search index '${this.indexName}' created successfully for collection '${this.collectionName}'.`,
          );
        }
      } catch (e: any) {
        console.warn(
      ...
      try {
        ...
        if (foundTextIndex) {
          console.log(
            `Text search index '${textIndexName}' already exists in collection '${this.collectionName}'.`,
          );
        } else {
          await this.collection.createSearchIndex({
            name: textIndexName,
          ...
          console.log(
            `Text search index '${textIndexName}' created successfully for collection '${this.collectionName}'.`,
          );
        }
      } catch (e: any) {
        console.warn(
  ...
  async insert(
    ...
    console.log(
      `Inserting ${vectors.length} vectors into collection '${this.collectionName}'.`,
    );
    ...
    try {
      ...
      console.log(
        `Inserted ${documents.length} documents into '${this.collectionName}'.`,
      );
    } catch (error) {
  ...
  async search(
    ...
    try {
      ...
      console.log(
        `Vector search completed. Found ${results.length} documents.`,
      );
  ...
  async keywordSearch(
    ...
    try {
      ...
      const results = await this.collection.aggregate(pipeline).toArray();
      console.log(
        `Keyword search completed. Found ${results.length} documents.`,
      );
  ...
  async get(vectorId: string): Promise<VectorStoreResult | null> {
    ...
    try {
      ...
      if (doc) {
        console.log(`Retrieved document with ID '${vectorId}'.`);
        return {
          id: String(doc._id),
          payload: doc.payload || {},
        };
      }
      console.log(`Document with ID '${vectorId}' not found.`);
      return null;
    } catch (error) {
    ...
    if (Object.keys(updateFields).length > 0) {
      try {
        ...
        if (result.matchedCount > 0) {
          console.log(`Updated document with ID '${vectorId}'.`);
        } else {
  ...
  async delete(vectorId: string): Promise<void> {
    ...
    try {
      ...
      if (result.deletedCount && result.deletedCount > 0) {
        console.log(`Deleted document with ID '${vectorId}'.`);
      } else {
  ...
  async list(
    ...
    try {
      ...
      console.log(
        `Retrieved ${results.length} documents from collection '${this.collectionName}'.`,
      );
